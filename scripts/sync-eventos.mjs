...
function timezoneFor(country, attrs = {}, league = '', description = '') {
  // 1) Always prefer an explicit IANA timezone supplied by the source.
  const explicit = String(attrs.timezone || attrs.time_zone || attrs.tz || '').trim();
  if (explicit) return explicit;

  // 2) Normal country mapping. This includes the aliases used by diaries.json.
  if (TZ[country]) return TZ[country];

  // 3) Some diaries are grouped under a generic category instead of a country.
  // These overrides are only used where the event itself identifies a clear venue/competition.
  const text = `${country} ${league} ${description}`.toLowerCase();
  if (/us open/.test(text)) return 'America/New_York';
  if (/indycar.*laguna seca|laguna seca.*indycar/.test(text)) return 'America/Los_Angeles';
  if (/ciclismo.*la vuelta|la vuelta.*ciclismo/.test(text)) return 'Europe/Madrid';
  if (/rugby.*top 14|top 14.*rugby|la rochelle.*toulouse/.test(text)) return 'Europe/Paris';
  if (/eerste divisie|eredivisie|heerenveen|az|telstar|cambuur|ado den haag|fortuna sittard|mvv|roda jc/.test(text)) return 'Europe/Amsterdam';

  // 4) Keep truly ambiguous worldwide categories in UTC rather than inventing a timezone.
  return 'UTC';
}
...
for (const item of source.data) {
  ...
  const tz = timezoneFor(country, a, leagueName, description);
