...
if (!source || !Array.isArray(source.data)) {
  throw new Error('Format agenda18.com/agenda.json tidak sesuai: properti "data" tidak ditemukan.');
}

const TZ = {
  'Copa Centroamericana': 'America/Guatemala',
...
for (const item of source.data) {
  ...
  if (colon > 0) {
    leagueName = title.slice(0, colon).trim();
    title = title.slice(colon + 1).trim();
  }
  if (!title) title = lines[0].replace(/:\s*$/, '');
