...
PLAYER_BASE = "https://vivo.totalh.net/en-vivo/tv.html?stream="
...
FLAG_CODE = {
    # Nama file disesuaikan persis dengan folder flags yang diberikan.
    ...
    "Italia": "it",
    "Países Bajos": "hol", "Netherlands": "hol",
    "México": "mx", "Mexico": "mx",
    "Perú": "pe", "Peru": "pe",
    "Portugal": "pt",
    "Paraguay": "py",
...
def fallback_logo(country_name, description):
    c = clean(country_name)
    d = clean(description).lower()
    if c.lower().startswith("mlb") or " mlb " in (" " + d + " "):
        return "https://raw.githubusercontent.com/omezing75-byte/data-sport-nexa-futbollibre/refs/heads/main/flags/mlb.png"
    code = FLAG_CODE.get(c)
    if code:
        return f"https://raw.githubusercontent.com/omezing75-byte/data-sport-nexa-futbollibre/refs/heads/main/flags/{code}.png"
    return ""
...
def main():
    ...
    for row in src.get("data", []):
        ...
        logo_fallback = fallback_logo(country_name, description)
        ...
        for embed_row in ((attrs.get("embeds") or {}).get("data") or []):
            ...
            embed_name = clean(ea.get("embed_name"))
            embed_iframe = clean(ea.get("embed_iframe"))
            player_url = ""

            # The source embed contains a base64 encoded destination in ?r=.
            # Decode it only to recover the stream key (e.g. espn3, tntsports,
            # winsports2), then point the schedule to the user's own player.
            try:
                parsed = urlparse(embed_iframe)
                encoded = parse_qs(parsed.query).get("r", [""])[0]
                if encoded:
                    decoded = base64.b64decode(encoded + "=" * (-len(encoded) % 4)).decode("utf-8", "ignore")
                    stream = parse_qs(urlparse(decoded).query).get("stream", [""])[0].strip()
                    if stream:
                        player_url = PLAYER_BASE + stream
            except Exception:
                player_url = ""
